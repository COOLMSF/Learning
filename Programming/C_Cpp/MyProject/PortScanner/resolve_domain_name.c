...
/*
 * This function resolve domain name from struct info_t,
 * and store resolved information in struct info_t.
 */
...
int resolve_domain_name(struct info_t *target_info_ptr) {
    ...
    for (ai_ptr = res; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next) {
        ...
        // Some addresses are the same, don't save them
        if (!has_network_addr(target_info_ptr, sin_ptr->sin_addr.s_addr)) {
             ...
             * machine. We can define a function to find how big it is. I just put 15 for convenient.
             */
