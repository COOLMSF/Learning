...
#diplay something
  movw $msg,%si
    c431:	c4                   	(bad)  
  call puts
    c432:	e8 5d 00 be 81       	call   81bec494 <__bss_start+0x81bdff0c>
  
  movw $try,%si
    c437:	c4                   	(bad)  
  call puts
    c438:	e8 57 00 fa fc       	call   fcfac494 <__bss_start+0xfcf9ff0c>
...
void bootmain(void){
    ...
    c4de:	b8 00 00 0a 00       	mov    $0xa0000,%eax
		//write_mem8(i, i&0x0f);
		p=(char *)i;
		*p=i&0x0f;
    c4e3:	89 c2                	mov    %eax,%edx
	for(i=0xa0000; i<=0xaffff; i++){
    c4e5:	40                   	inc    %eax
		*p=i&0x0f;
    c4e6:	83 e2 0f             	and    $0xf,%edx
    c4e9:	88 50 ff             	mov    %dl,-0x1(%eax)
	for(i=0xa0000; i<=0xaffff; i++){
    c4ec:	3d 00 00 0b 00       	cmp    $0xb0000,%eax
    c4f1:	75 f0                	jne    c4e3 <bootmain+0x5>
	}

	for(;;){
		io_halt();
    c4f3:	f4                   	hlt    
    c4f4:	eb fd                	jmp    c4f3 <bootmain+0x15>
...
{
    c4f6:	55                   	push   %ebp
    c4f7:	89 e5                	mov    %esp,%ebp
    c4f9:	0f b6 55 08          	movzbl 0x8(%ebp),%edx
  int i;
  for(i=0xa0000;i<0xaffff;i++)
    c4fd:	b8 00 00 0a 00       	mov    $0xa0000,%eax
  {
  write_mem8(i,color);  //if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
    c502:	88 10                	mov    %dl,(%eax)
  for(i=0xa0000;i<0xaffff;i++)
    c504:	83 c0 01             	add    $0x1,%eax
    c507:	3d ff ff 0a 00       	cmp    $0xaffff,%eax
    c50c:	75 f4                	jne    c502 <clear_screen+0xc>
...
void color_screen(char color) //15:pure white
{
  ...
  color=color;
  for(i=0xa0000;i<0xaffff;i++)
    c510:	b8 00 00 0a 00       	mov    $0xa0000,%eax
  {
  write_mem8(i,i&0x0f);  //if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
    c515:	89 c2                	mov    %eax,%edx
    c517:	83 e2 0f             	and    $0xf,%edx
    c51a:	88 10                	mov    %dl,(%eax)
  for(i=0xa0000;i<0xaffff;i++)
    c51c:	83 c0 01             	add    $0x1,%eax
    c51f:	3d ff ff 0a 00       	cmp    $0xaffff,%eax
    c524:	75 ef                	jne    c515 <color_screen+0x5>
