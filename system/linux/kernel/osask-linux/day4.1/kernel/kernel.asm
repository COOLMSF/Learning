...
0000c481 <try>:
    ...
    c486:	72 79                	jb     c501 <clear_screen+0xd>
...
0000c4f4 <clear_screen>:
#include<header.h>

void clear_screen(char color) //15:pure white
{
    c4f4:	55                   	push   %ebp
    c4f5:	89 e5                	mov    %esp,%ebp
    c4f7:	0f b6 55 08          	movzbl 0x8(%ebp),%edx
  int i;
  for(i=0xa0000;i<0xaffff;i++)
    c4fb:	b8 00 00 0a 00       	mov    $0xa0000,%eax
  {
  write_mem8(i,color);  //if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
    c500:	88 10                	mov    %dl,(%eax)
  for(i=0xa0000;i<0xaffff;i++)
    c502:	83 c0 01             	add    $0x1,%eax
    c505:	3d ff ff 0a 00       	cmp    $0xaffff,%eax
    c50a:	75 f4                	jne    c500 <clear_screen+0xc>
...
void color_screen(char color) //15:pure white
{
  ...
  write_mem8(i,i&0x0f);  //if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
    c513:	89 c2                	mov    %eax,%edx
    c515:	83 e2 0f             	and    $0xf,%edx
    c518:	88 10                	mov    %dl,(%eax)
  for(i=0xa0000;i<0xaffff;i++)
    c51a:	83 c0 01             	add    $0x1,%eax
    c51d:	3d ff ff 0a 00       	cmp    $0xaffff,%eax
    c522:	75 ef                	jne    c513 <color_screen+0x5>
