...
#diplay something
  movw $msg,%si
    c431:	c4                   	(bad)  
  call puts
    c432:	e8 5d 00 be 81       	call   81bec494 <__bss_start+0x81bdff20>
  
  movw $try,%si
    c437:	c4                   	(bad)  
  call puts
    c438:	e8 57 00 fa fc       	call   fcfac494 <__bss_start+0xfcf9ff20>
...
0000c481 <try>:
    ...
    c486:	72 79                	jb     c501 <color_screen+0x7>
...
#include<header.h>
void bootmain(void){
    c4de:	eb fe                	jmp    c4de <bootmain>

0000c4e0 <clear_screen>:
#include<header.h>
...
void clear_screen(char color) //15:pure white
{
    ...
    c4e3:	0f b6 55 08          	movzbl 0x8(%ebp),%edx
  int i;
  for(i=0xa0000;i<0xaffff;i++)
    c4e7:	b8 00 00 0a 00       	mov    $0xa0000,%eax
  {
  write_mem8(i,color);  //if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
    c4ec:	88 10                	mov    %dl,(%eax)
  for(i=0xa0000;i<0xaffff;i++)
    c4ee:	83 c0 01             	add    $0x1,%eax
    c4f1:	3d ff ff 0a 00       	cmp    $0xaffff,%eax
    c4f6:	75 f4                	jne    c4ec <clear_screen+0xc>
...
void color_screen(char color) //15:pure white
{
  ...
  color=color;
  for(i=0xa0000;i<0xaffff;i++)
    c4fa:	b8 00 00 0a 00       	mov    $0xa0000,%eax
  {
  write_mem8(i,i&0x0f);  //if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
    c4ff:	89 c2                	mov    %eax,%edx
    c501:	83 e2 0f             	and    $0xf,%edx
    c504:	88 10                	mov    %dl,(%eax)
  for(i=0xa0000;i<0xaffff;i++)
    c506:	83 c0 01             	add    $0x1,%eax
    c509:	3d ff ff 0a 00       	cmp    $0xaffff,%eax
    c50e:	75 ef                	jne    c4ff <color_screen+0x5>
