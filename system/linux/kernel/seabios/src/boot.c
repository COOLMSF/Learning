...
boot_disk(u8 bootdrv, int checksig)
{
    ...
    memset(&br, 0, sizeof(br));
    br.flags = F_IF;
    br.dl = bootdrv; // device number
    br.es = bootseg; // ES:BX destination
    br.ah = 2; // read sector
    br.al = 1; // Sectors To Read Count
    br.cl = 1; // Sector

    /*
     * Read disk, but from where to where?
     * google int 0x13
     */
    call16_int(0x13, &br);
    ...
    bootseg &= 0xf000;
    // bootip = 0x7c00, which is where mbr located
    // bootseg = 0x0
